  ...
  const addToObjectStore = (objectStore, data) => {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = function (event) {
        reject();
        return (
          <ErrorMessage title="IndexedDB Error" desc="GeneCodex could not send your file to your browser's Indexed Database. Please try again." />
        )
      };
    });
  };
  ...
  const uploadToIndexedDB = async (event) => {
    ...
    request.onsuccess = function (event) {
      ...
      getRequest.onsuccess = function (event) {
        ...
        dna.parse(fileContent, function (err, snps) {
          if (err) {
            console.log(err);
            return (
              <ErrorMessage title="Parsing Error" desc="GeneCodex could not parse your file successfully. Please try again." />
            )
          } else {
            const data = {
              id: 'snpsData',
            ...
            addRequest.onsuccess = function (event) {
              console.log('SNPs data saved to IndexedDB successfully');
              setIsProcessing(false);
              setIsOpen(true);
            };
  
            addRequest.onerror = function (event) {
              console.log('Error saving SNPs data to IndexedDB');
              setIsProcessing(false);
              setIsOpen(true);
              return (
                <ErrorMessage title="IndexedDB Error" desc="GeneCodex could not send your file to your browser's Indexed Database. Please try again." />
      ...
      getRequest.onerror = function (event) {
        console.log('Error retrieving file from IndexedDB');
        setIsProcessing(false);
        setIsOpen(true);
        return (
        ...
        <div className="flex items-center justify-center w-full px-4 sm:px-0">
            <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition ease-in-out duration-150">
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-xs text-gray-500 text-center">23andMe, AncestryDNA, or FamilyTreeDNA raw data .txt files</p>
                </div>
